...
CREATE TABLE member_coupon (
    ...
    status              VARCHAR(30) COLLATE utf8mb4_0900_as_cs  NOT NULL DEFAULT 'ISSUED', -- 발급분 상태(ISSUED 발급/USED 사용/EXPIRED 만료/CANCELED 주문 취소로 사용 철회). CANCELED 는 조회 시 앱이 ISSUED 나 EXPIRED 로 해소한다
    ...
    /*
     * 사용 시각은 사용 상태에만 있다.
     * CANCELED 로 바꿀 때 used_at 을 함께 비워야 한다. 사용이 철회됐으므로 사용 시각도 남지 않는다.
...
CREATE TABLE member_coupon_status_history (
    ...
    CONSTRAINT chk_mcsh_to_status CHECK (to_status IN ('ISSUED','USED','EXPIRED','CANCELED')), -- member_coupon.status 와 같은 집합. 한쪽만 고치면 전이가 막힌다
    CONSTRAINT chk_mcsh_from_status CHECK (from_status IS NULL OR from_status IN ('ISSUED','USED','EXPIRED','CANCELED')), -- member_coupon.status 와 같은 집합
