 ...
 *
 * 지금은 이 Aspect의 대상이 되는 {@code @Scheduled} 메서드가 프로젝트에 하나도 없다. 그래도 이
 * Aspect 자체는 스케줄러 구현과 무관한 재사용 가능한 공통 인프라라 미리 추가해 둔다 — 나중에
 * 스케줄러가 생기면 {@code @Scheduled}만 붙이면 바로 적용된다.
 */
@Slf4j
@Aspect
@Component
