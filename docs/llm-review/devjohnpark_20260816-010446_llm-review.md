...
없음. 이 범위(`efe9eff..371b877`, 커밋 1개)에 해당하는 `371b877` 은 파일을 하나도 바꾸지 않은 빈 커밋이다
(`git show --stat`: 메시지 외 변경 없음). 변경 파일 0건이라 어떤 `trigger` 글롭도 걸리지 않아
`defaults.on_no_match` 가 적용되었다: `levels: [코드]`, `prefixes: [EJ]`.
