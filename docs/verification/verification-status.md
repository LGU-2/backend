...
| `build.gradle`, `settings.gradle` | build | 35 | 9 | 26 | 9 | 26 | 0 |
...
585건 중 **482건**이 어떤 규칙엔가 걸린다. 나머지 103건은 아래와 같다.
...
backend 272건, infra 96건 의 층위 태그가 기본값으로 채워져 있다. 실제와 다른 것이 섞여 있어 `levels` 필터가 정확하지 않다.
