...
**처음이라면** 아래 [준비](#준비) 를 먼저 한다. 옆에 없으면 `/v-local` 가 물어보고 받는다.
...
```
커밋하면      로컬에서 Claude 가 본다   (/v-local 를 쳐야 돈다)
...
되짚어 볼 필요가 있으면 로컬에서 `/v-local` 를 돌려 기록을 남긴다.
...
| 증상 | 조치 |
|---|---|
| `/v-local` 가 기준 저장소를 받겠다고 묻는다 | 처음 한 번만 묻는다. 승인한다 |
