...
> **현재 정책은 ORM 생성이며 사전 채번은 기본 경로가 아니다.** (guideline 5절)
> 이 절은 사전 채번이 언제 값을 하는지를 남긴 것이며, 아래 두 흐름이 실제로 필요해지면
> `IDS-5-05`의 예외 경로로 값을 미리 넣는다. 그때 이 절이 판단 근거가 된다.
...
| 초판의 반대 근거 | 지금 |
|---|---|
| Hibernate 구현이 rand_a 12비트를 서브밀리초로, 62비트를 단조 카운터로 쓴다 | **해소.** 내장 `UuidVersion7Strategy`를 안 쓰고 `@ValueGenerationType`으로 우리 생성기를 붙인다. 4절 규칙이 그대로 적용된다 |
| `public_id`는 `@Id`가 아니라 식별자 생성기의 대상이 아니다 | **해소.** `@UuidGenerator`가 아니라 커스텀 값 생성기를 쓴다 |
| 값이 persist 시점에 채워져 사전 채번이 불가능하다 | **남는다.** 3절의 두 흐름을 포기했다 |
| 테스트에서 값을 고정할 수 없다 | **부분 해소.** `IDS-5-05`가 이미 값이 있으면 덮어쓰지 않으므로 지정할 수 있다 |

**세 번째가 이 결정의 대가다.** 결제 준비처럼 저장 전에 외부로 식별자를 넘겨야 하는 흐름이 생기면,
`IDS-5-05`의 경로로 값을 미리 넣고 그 사유를 PR에 남긴다. 기본 경로를 벗어나는 것이므로 예외로 다룬다.

JPA 표준의 `GenerationType.UUID`는 명세가 버전을 규정하지 않으며 Hibernate는 v4로 구현한다.
Hibernate 내장 `@UuidGenerator(style = VERSION_7)`도 쓰지 않는다. 생성 시점만 ORM에 맡기고 **생성 방식은 우리가 정한다.**
