...
점검 항목
* 도메인 루트에 API 인터페이스, record DTO, 공개 예외 외의 것이 없는가
  `@Entity`, Repository, Controller, 구현 클래스가 루트에 올라오면 다른 도메인이 영속 모델에 직접 의존하게 된다.
* 다른 도메인의 `domain` 패키지를 import하지 않는가
* API를 호출당하지 않는 도메인에도 의무적으로 만들지 않았는가
  쓰지 않는 API를 미리 만들면 유지할 계약만 늘고 아무도 소비하지 않는다.
* `~ApiImpl`이 계층 안이 아니라 `domain` 바로 아래에 있는가
...
점검 항목
* 공개 DTO에 엔티티를 그대로 담지 않았는가
  `record ProductInfo(Product product)`는 감춘 것이 없다. 필드를 값으로 풀어 담는다.
* 공개 DTO가 내부 enum을 그대로 노출하지 않는가
  노출하면 그 enum이 계약의 일부가 되어 상수 추가 시 다른 도메인 영향을 검토해야 한다.
* 도메인 예외를 통합 예외 하나와 에러 코드 enum으로 두었는가
  실패 종류마다 예외 클래스를 만들면 클래스 수가 계속 늘어난다.
* 다른 도메인이 구분해 처리해야 하는 실패만 별도 예외로 루트에 공개했는가
* 에러 코드를 `common`의 enum 하나에 모으지 않았는가
...
점검 항목
* 인터페이스를 도메인이 소유하고 구현체가 외부 스펙에 맞추는가
  `PaymentService`는 `PgClient`만 알고 어느 PG사인지 모른다.
* 외부 스펙 DTO를 도메인 밖으로 내보내지 않는가
  `client.dto`에 가두고 도메인이 쓰는 형태로 변환해 넘긴다.
* 최상위에 `client` 패키지를 두지 않았는가
  모든 도메인이 접근 가능해져 경계가 무너진다.
* `client` 클래스에 `@Transactional`이 없는가
...
점검 항목
* 공개 창구 이름이 도메인명 + `Api` 형태인가
* 구현체 이름에 패턴명(`Facade` 등)을 넣지 않았는가
* 자기 도메인 컨트롤러가 API를 경유하지 않는가
  같은 `domain` 안이므로 내부 서비스를 직접 쓴다. API는 도메인 간 협력 전용이다.
* 통과 위임만 하는 API 구현체가 아닌가
  변환이나 노출 범위 축소가 없으면 노이즈다.
* API 구현체에 비즈니스 로직이 없는가
...
점검 항목
* `~ApiImpl`과 `Controller`가 package-private인가
  Spring은 package-private 빈도 정상적으로 등록하고 주입한다.
* 계층 간 참조가 없는데 public으로 선언하지 않았는가
...
점검 항목
* ArchUnit 테스트가 빌드 파이프라인에 묶여 있는가
  계층 구조에서는 테스트가 유일한 자동 검증 수단이다.
* `consideringOnlyDependenciesInLayers()`가 적용되어 있는가
  없으면 계층으로 선언하지 않은 패키지와의 의존까지 검사되어 오탐이 발생한다.
* `entity`, `dto`, `exception`을 계층으로 선언하지 않았는가
...
점검 항목
* 조회성 호출을 트랜잭션 시작 전으로 뺐는가
* 외부 시스템 호출이 트랜잭션 밖에 있는가
  DB 락을 잡은 채 네트워크 대기를 하지 않는다.
* 트랜잭션이 필요한 DB 쓰기만 별도 메서드(다른 빈)로 분리했는가
  같은 클래스 내부 호출은 프록시를 타지 않아 `@Transactional`이 무시된다.
* 여러 도메인에 걸친 쓰기의 호출 순서를 고정했는가
  교착 가능성을 줄인다.
* 경합이 심한 쓰기에 잠금 전략이 명시되어 있는가
  짧고 확실한 경합은 비관적 락, 충돌이 드문 경우는 낙관적 락(`@Version`)을 쓴다.
