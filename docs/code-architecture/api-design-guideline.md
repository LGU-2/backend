...
점검 항목
* `API-2-01` API를 리소스 계층(컬렉션과 하위 컬렉션)으로 모델링했는가 (AIP-121)
* `API-2-02` API 표면을 내부 DB 스키마와 동일하게 노출하지 않았는가 (AIP-121)
  DB 구조를 그대로 드러내면 표면이 내부 구현에 강하게 묶인다.
* `API-2-03` 리소스 이름이 계층을 드러내는 경로 형식인가 (AIP-122)
  예: `publishers/{publisher}/books/{book}`. 컬렉션 ID는 복수형을 쓴다.
* `API-2-04` 리소스 간 참조와 부모 자식 관계가 순환 없이 단방향(비순환 그래프)인가 (AIP-121, AIP-124)
* `API-2-05` enum을 적절히 정의하고 0번 값을 unspecified로 두었는가 (AIP-126)
* `API-2-06` 서버가 채우는 값과 기본값을 명확히 구분했는가 (AIP-129)
* `API-2-07` 단일 인스턴스 리소스는 싱글톤으로 설계했는가 (AIP-156)
* `API-2-08` 관리자용 경로가 `/v1/admin/` 아래에 모여 있는가
  같은 리소스라도 소비자가 다르면 표면이 갈린다. 관리자 경로가 회원 경로에 섞이면
  인증 방식과 노출 필드가 경로만으로 구분되지 않는다.
  클래스 이름은 이 경로를 그대로 따른다(`domain-package-boundary-guideline.md` 4.2절).
...
점검 항목
* `API-3-01` 표준 메서드(Get, List, Create, Update, Delete)로 표현 가능한 동작을 커스텀으로 만들지 않았는가 (AIP-130)
* `API-3-02` 리소스가 최소한 Get을 지원하는가, 그리고 싱글톤이 아니면 List도 지원하는가 (AIP-121)
* `API-3-03` Get은 부수 효과가 없는가 (AIP-131)
* `API-3-04` List는 페이지네이션을 제공하는가 (AIP-132, AIP-158)
* `API-3-05` Create는 부모 컬렉션에 대해 수행하고 생성된 리소스를 반환하는가 (AIP-133)
* `API-3-06` Update는 필드 마스크로 부분 수정을 지원하는가 (AIP-134, AIP-161)
* `API-3-07` Delete의 응답과 멱등성, 그리고 소프트 삭제 여부를 검토했는가 (AIP-135, AIP-164)
* `API-3-08` 커스텀 메서드는 표준 HTTP 동사(주로 POST)에 콜론 표기로 동사를 붙였는가 (AIP-136)
  예: `POST /v1/publishers/123/books/456:publish`
* `API-3-09` 오래 걸리는 작업은 장기 실행 오퍼레이션으로 처리하고 폴링 가능한 핸들을 반환하는가 (AIP-151)
* `API-3-10` 여러 건을 한 번에 처리하는 배치 메서드는 전부 성공 또는 전부 실패(원자성)를 따르는가 (AIP-231~235)
...
점검 항목
* `API-4-01` 필드 이름이 같은 개념에 같은 이름을 쓰는가 (AIP-140)
* `API-4-02` 전치사를 넣지 않았는가 (AIP-140)
  예: `error_reason`(권장), `reason_for_error`(지양)
* `API-4-03` 필드 이름이 동사가 아니라 명사인가 (AIP-140)
  필드는 의도가 아니라 값을 나타낸다. `disabled`(권장), `disable`(지양)
* `API-4-04` 반복 필드는 복수형, 단일 필드는 단수형인가 (AIP-140, AIP-144)
* `API-4-05` boolean 필드에 "is" 접두어를 붙이지 않았는가 (AIP-140)
  예: `disabled`(권장), `is_disabled`(지양)
* `API-4-06` 수량 필드는 단위 접미어를 붙였는가 (AIP-141)
  예: `distance_km`, `width_px`. 비율에는 "per"를 쓴다.
* `API-4-07` 시간과 기간을 표준 타입으로 표현하고 이름을 `*_time`, `*_duration`으로 두었는가 (AIP-142)
* `API-4-08` 언어, 통화, 국가 등은 표준 코드(BCP-47, ISO 4217, ISO 3166)를 쓰는가 (AIP-143)
* `API-4-09` 범위 필드의 포함과 배제 경계를 명확히 했는가 (AIP-145)
* `API-4-10` 임의 구조 데이터(Struct, Any 등)는 꼭 필요할 때만 제한적으로 쓰는가 (AIP-146)
* `API-4-11` 민감 정보는 입력 전용으로 두고 응답에 반환하지 않는가 (AIP-147)
* `API-4-12` 표준 필드(name, create_time, update_time, uid, display_name 등)는 정해진 의미와 타입으로 쓰는가 (AIP-148)
* `API-4-13` 값의 미설정과 기본값을 구분해야 하는 경우를 처리했는가 (AIP-149)
* `API-4-14` 필드 동작(REQUIRED, OPTIONAL, OUTPUT_ONLY, IMMUTABLE, INPUT_ONLY)을 문서화했는가 (AIP-202, AIP-203)
* `API-4-15` 상태 필드는 `state`로 두고 출력 전용이며 전이 규칙을 문서화했는가, 상태에 동작을 섞지 않았는가 (AIP-216)
...
점검 항목
* `API-5-01` 컬렉션 반환은 처음부터 페이지네이션을 넣었는가 (AIP-158)
  나중에 추가하면 호환성이 깨진다. `page_size`, `page_token` 요청 필드와 `next_page_token` 응답 필드를 쓴다.
* `API-5-02` page_token은 클라이언트가 해석할 수 없는 불투명 문자열인가 (AIP-158)
* `API-5-03` 컬렉션 횡단 조회는 부모에 와일드카드(`-`)를 쓰는가 (AIP-159)
* `API-5-04` 필터링은 `filter` 문자열 필드로 표준 문법을 따르는가 (AIP-160)
* `API-5-05` 부분 수정과 부분 응답에 필드 마스크를 쓰는가 (AIP-161, AIP-157)
* `API-5-06` 갱신 손실을 막기 위해 etag로 리소스 신선도를 검증하는가 (AIP-154)
* `API-5-07` 재시도 시 중복 처리를 막기 위해 요청 식별자(request_id)로 멱등성을 보장하는가 (AIP-155)
* `API-5-08` 실제 반영 없이 검증만 하는 validate_only(드라이런)를 제공하는가 (AIP-163)
* `API-5-09` 삭제는 소프트 삭제로 처리하고 undelete, 만료, show_deleted 목록 옵션을 검토했는가 (AIP-164)
* `API-5-10` 조건 기반 일괄 삭제는 위험을 고려해 신중히 설계했는가 (AIP-165)
* `API-5-11` 가져오기와 내보내기는 커스텀 메서드(:import, :export)로 표현했는가 (AIP-153)
* `API-5-12` 반복 실행 단위는 잡(Job) 패턴으로 분리했는가 (AIP-152)
* `API-5-13` 리소스 만료가 필요하면 expire_time이나 ttl을 제공하는가 (AIP-214)
* `API-5-14` 일부 위치를 조회하지 못할 수 있으면 List 응답에 unreachable을 두는가 (AIP-217)
* `API-5-15` 인가를 먼저 검사하고, 그다음 존재 여부를 확인하는가 (AIP-211, AIP-193)
* `API-5-16` 텍스트는 UTF-8로 다루는가 (AIP-210)
...
점검 항목
* `API-6-01` 이번 변경이 하위 호환을 깨는 변경인지 확인했는가 (AIP-180)
  필수 필드 추가, 필드 제거나 이름 변경, 타입 변경, 기존 메서드에 페이지네이션 추가, 검증 강화 등이 호환을 깬다.
* `API-6-02` 안정성 단계(alpha, beta, stable)를 명확히 했는가 (AIP-181)
* `API-6-03` 호환을 깨는 변경은 새 메이저 버전으로 분리했는가 (AIP-185)
  버전은 경로에 둔다. 예: `/v1/...`, `/v2/...`
* `API-6-04` API 계약이 외부 소프트웨어에 불필요하게 의존하지 않는가 (AIP-182)

## 7. 마무리(Polish)

점검 항목
* `API-7-01` 명명 규칙을 API 전체에서 일관되게 따랐는가 (AIP-190)
* `API-7-02` 모든 메서드, 리소스, 필드에 문서 주석을 달았는가 (AIP-192)
* `API-7-03` 오류 응답이 표준 구조와 표준 코드를 따르는가 (AIP-193)
  표준 상태 코드를 쓰고, 기계 판독용 식별자(reason, domain)와 동적 정보(metadata)를 포함한다.
* `API-7-04` 오류 메시지가 간결하면서 해결 방향을 제시하고, 내부 구현이나 민감 정보를 노출하지 않는가 (AIP-193)
* `API-7-05` 권한이 없으면 존재 여부와 무관하게 PERMISSION_DENIED(403)를, 권한은 있으나 없으면 NOT_FOUND(404)를 반환하는가 (AIP-193, AIP-211)
* `API-7-06` 어떤 오류 코드가 재시도 가능한지 정의했는가 (AIP-194)
...
점검 항목
* `API-8-01` HTTP와 gRPC 매핑을 google.api.http 애너테이션으로 정의했는가 (AIP-127)
* `API-8-02` 공통 타입(google.rpc, google.type 등)을 재사용했는가 (AIP-213)
* `API-8-03` API용 proto를 내부 저장용 proto와 분리했는가 (AIP-215)
* `API-8-04` proto 파일과 디렉터리 구조가 규칙을 따르는가 (AIP-191)
