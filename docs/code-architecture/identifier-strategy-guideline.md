...
점검 항목
* 하나의 컬럼이 두 층의 역할을 겸하지 않는가
  짧고 읽기 쉬우면 추측 가능하고, 추측 불가능하면 전화로 부를 수 없다. 요구 조건이 서로 모순된다.
* 리소스가 여러 식별자를 가진다면 각각의 소비자와 용도가 문서에 명시되어 있는가
...
점검 항목
* 구성 선택의 근거가 기록되어 있는가
* 새 엔티티가 아래 기준에 해당하는데 `public_id`를 누락하지 않았는가
* 반대로 기준에 해당하지 않는데 습관적으로 달지 않았는가
...
점검 항목
* 난수원이 `SecureRandom`인가
  `java.util.Random`은 시드가 48비트뿐이라 몇 개의 출력만 관측해도 내부 상태가 복원된다. `ThreadLocalRandom`도 금지한다.
* `SecureRandom.getInstanceStrong()`을 쓰지 않았는가
  리눅스에서 블로킹 엔트로피 소스에 연결될 수 있어 컨테이너 기동 직후 애플리케이션이 통째로 정지한다.
* 외부 UUID 라이브러리를 도입했다면 난수원을 확인했는가
  일부 v7 구현체가 속도를 위해 rand_b를 비암호학적 PRNG로 채운다.
* RFC 9562의 단조 카운터 옵션(Method 1, 2)이나 rand_a 서브밀리초 대체(Method 3)를 쓰지 않았는가

## 5. 생성 시점

점검 항목
* 식별자가 `@PrePersist`나 ORM 생성기가 아니라 **생성자 인자로** 확정되는가
  persist 시점에 채워지면 저장 전까지 `null`이라 사전 채번의 이점이 사라지고, 테스트에서 값을 고정할 수 없다.
* 생성을 `PublicIdGenerator` 인터페이스 뒤에 두었는가
  테스트 대역을 둘 수 있는 것이 이 인터페이스의 존재 이유다.
* `@UuidGenerator`나 `GenerationType.UUID`를 쓰지 않았는가
...
점검 항목
* `public_id`가 `BINARY(16)`이고 `NOT NULL`인가
* `DEFAULT (UUID())`를 걸지 않았는가
* FK와 조인 조건이 내부 식별자를 쓰는가
...
점검 항목
* 응답 DTO나 API 경로에 내부 `Long id`가 없는가
  한 번 노출되면 클라이언트가 의존하기 시작해 되돌릴 수 없다.
* 대량 조회 경로가 외부 식별자 목록으로 돌지 않는가
* 응답 DTO의 필드명이 `publicId`가 아니라 `id`인가
  클라이언트에게는 그것이 유일한 식별자다. 다른 ID의 존재를 알릴 이유가 없다.

## 8. 타입과 예외

점검 항목
* 외부 입력 UUID 파싱 실패가 400으로 매핑되는가
  `UUID.fromString`의 `IllegalArgumentException`을 그대로 두면 500이 나간다.
* `AbstractPublicId` 하위 타입이 상태를 추가하지 않는가
  추가하면 `getClass()` 기반 equals 전제가 깨진다.

## 9. 정렬

점검 항목
* UUID를 정렬 키나 커서 페이지네이션 키로 쓰지 않는가
